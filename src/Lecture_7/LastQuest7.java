 ...
 * чтобы проверять определенные инварианты в коде. С помощью него возможно писать небольшие тесты и следить
 * за корректностью своей программы (в обычной ситуации предпочтительно
 ...
 * параметры виртуальной машины. Сделать это можно прямо
 * при запуске в консоли с помощью программы java, либо указав этот ключ в настройках запуска программы в вашей IDE.
 * В случае IntellijIDEA, например,
 ...
 * <p>
 * В конечном итоге, вам нужно реализовать классы MailService, MailMessage и Salary (и, вероятно,
 * вспомогательные классы и интерфейсы) и отправить их код в форму.
...
public class LastQuest7 {
    ...
    public static class Salary extends AbstractSendable {
        ...
        public Salary(String from, String to, int salaryValue) {
            super(from, to);
            this.content = salaryValue;
        }
    ...
    public static class MailService<T> implements Consumer<Sendable> {
        ...
        private final Map<String, List<T>> mapMailMessage = new MailBox<>();


        @Override
        public void accept(Sendable mailPackage) {

            if (mapMailMessage.get(mailPackage.getTo()) == null) {
                mapMailMessage.put(mailPackage.getTo(), new ArrayList<T>(Arrays.asList(mailPackage.getContent())));
            } else {
                List<T> list = mapMailMessage.get(mailPackage.getTo());
                list.add(mailPackage.getContent());
                mapMailMessage.put(mailPackage.getTo(), list);
            }
        }
    ...
    public static class MailBox<T, L> extends HashMap<T, List<L>> {
